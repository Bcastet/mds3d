...
void render(Scene* scene, ImageBlock* result, std::string outputName, bool* done)
{
    ...
    for(uint i=0; i<camera->vpWidth() ;i++){
      for(uint j=0;j<camera->vpHeight() ;j++){

        float alpha = i / float(camera->vpWidth());
        alpha = alpha*2 -1;
        float beta = j / float(camera->vpHeight());
        beta = beta*2 -1;

        Vector3f d = camF + alpha*camX + beta*camY;
        d.normalize();

        Ray ray = Ray( camera->position() , d);


        Color3f c = integrator->Li(scene, ray);

        Vector2f pos = Vector2f(i,j);
        result->put(pos,c);
      }
    }
